...
function Slider(props) {
  ...
  return (
    <div className={`overflow-hidden block ${styles.slider_container}`}>
      {window.innerWidth <= 820 && window.innerWidth > 576 && (
        <div
          className={`flex transition-transform ease-out duration-500`}
          style={{ transform: `translateX(-${props.curr * 10}%)` }}
        >
      ...
      {window.innerWidth <= 576 && (
        ...
        >
          {props.photos?.map((item, index) => (
            <div className={styles["mobile-image"]} key={index}>
              {item.popularity > 50 ? (
                <span
                  className={`bg-blue-600 text-white ml-12 lg:ml-16 p-1 absolute top-0 rounded-md`}
                >
      ...
      {window.innerWidth > 820 && (
        <div
          className={`flex transition-transform ease-out duration-500`}
          style={{ transform: `translateX(-${props.curr * 5}%)` }}
        >
          {props.photos?.map((item, index) => (
            <div
              className={styles["mobile-image"]}
